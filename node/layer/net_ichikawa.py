...
class net:
	...
	def dense(self, hidden=1024, depth=1, activation=tf.nn.tanh, dropout=False, parent=-1, bn=False):  #
		...
		width = hidden
		while depth > 0:
			with tf.name_scope('Dense_10'.format(hidden)) as scope:
	...
	def train(self, data=0, steps=-1, dropout=None, display_step=10, test_step=100, batch_size=10,
	          resume=save_step,ckpt_name="ichikawa.ckpt"):  # epochs=-1, ichikawa:ichikawa.ckpt
		...
		while step < steps:
			...
			if step % 5000 == 0 and step > 0:
				saver.save(session, ckpt_name.replace(".","_"+str(step)+"."))
...
#ichikawa
	...
	def train_ichikawa(self, data=0, steps=-1, dropout=None, display_step=10, test_step=100, batch_size=10,
		...
		if not resume or not self.resume(session):
			session.run([tf.global_variables_initializer()])
		#saver = tf.train.Saver([v for v in tf.all_variables() if not(v.name.startswith("model/prediction/Dense_10"))])
		saver = tf.train.Saver(tf.global_variables())
		saver.restore(session,"ichikawa.ckpt") 
		snapshot = self.name + str(get_last_tensorboard_run_nr())
		step = 0  # show first
		...
		#session.run(w_sh)
		check_1 = session.graph.get_tensor_by_name("model/conv_3/filters:0")
		w_sh_1 = tf.assign(check_1,tf.random_normal([3, 3, 76, 12]))
		session.run(w_sh_1)

		check_2 = session.graph.get_tensor_by_name("model/conv_4/filters:0")
		w_sh_2 = tf.assign(check_2,tf.random_normal([1, 1, 88, 88]))
		session.run(w_sh_2)

		check_3 = session.graph.get_tensor_by_name("model/conv_5/filters:0")
		w_sh_3 = tf.assign(check_3,tf.random_normal([3, 3, 88, 12]))
		session.run(w_sh_3)
	...
	def train_ichikawa_2(self, data=0, steps=-1, dropout=None, display_step=10, test_step=100, batch_size=10,
	          resume=save_step,ckpt_name="SSL.ckpt",start_ckpt="ichikawa.ckpt"):  # epochs=-1,
		print("learning_rate: %f" % self.learning_rate)
	...
	def accuracy_test(self, data=0, steps=-1, dropout=None, display_step=10, test_step=100, batch_size=10,
	          resume=save_step,ckpt_name="SSL.ckpt"):  # epochs=-1,
		import ros
		...
		while step < steps:
			...
			output = session.run([self.output,self.target],feed_dict=feed_dict)
